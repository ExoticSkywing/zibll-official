...
if (_pz('display_wp_update')) {
    ...
    add_action('admin_menu', 'zib_display_wp_update_notices'); // 禁用更新通知
...
{
    remove_action('admin_notices', 'update_nag', 3); //禁用更新通知
...
function zib_brush_limit_comment($approved, $commentdata)
{
    ...
    $user_id           = !empty($commentdata['user_id']) ? $commentdata['user_id'] : 0;
    $comment_author_IP = !empty($commentdata['comment_author_IP']) ? $commentdata['comment_author_IP'] : 0;
...
function zib_get_image_id($img_url)
{
    ...
    if ($post_id === false) {
        ...
        if (stristr($img_url, $baseurl)) {
            ...
            if ($sql_1) {
                global $wpdb;
                $db_results = $wpdb->get_results($wpdb->prepare("SELECT id FROM $wpdb->posts WHERE `post_type` = 'attachment' AND `guid` in ('%s', '%s') LIMIT 2", 'https:' . $sql_1, 'http:' . $sql_1));
                if ($db_results) {
