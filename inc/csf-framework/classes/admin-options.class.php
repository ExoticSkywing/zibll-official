...
if (!class_exists('CSF_Options')) {
  class CSF_Options extends CSF_Abstract
  {
    ...
    public function set_options($ajax = false)
    {
      ...
      if (wp_verify_nonce($nonce, 'csf_options_nonce')) {
        ...
        if (!empty($transient['reset'])) {
          ...
          //添加挂钩
          do_action("csf_{$this->unique}_reset_before", $data, $this);
          ...
          $this->notice = esc_html__('Default settings restored.', 'csf');
          //添加挂钩
          do_action("csf_{$this->unique}_reset_section_before", $data, $this);
        } else {
      ...
      if ($menu_type === 'submenu') {
        ...
        if (!empty($this->args['show_sub_menu']) && count($this->pre_tabs) > 1) {
          ...
          foreach ($this->pre_tabs as $section) {
            //修改部分，去除<badge>NEW</badge> 兼容.$new_badge['8.2']
            $_title =  preg_replace('/<badge\b[^>]*>(.*?)<\/badge>/is', '', $section['title']);

            call_user_func('add_submenu_page', $menu_slug, esc_attr($_title),  esc_attr($_title), $menu_capability, $menu_slug . '#tab=' . sanitize_title($_title), '__return_null');
          }
    ...
    public function add_options_html()
    {
      ...
      //额外添加按钮
      echo '<a href="javascript:;" class="csf-menu but hide-box c-red" title="显示选项菜单"><i class="fas fa-bars"></i></a>';
      echo '</div>';
      ...
      if ($has_nav) {
        ...
        foreach ($this->pre_tabs as $tab) {
          ...
          //试配加了badge之后的链接有效
          $tab_id    = sanitize_title(preg_replace("/<badge(.+?)<\/badge>/si",'',$tab['title']));
          ...
          if (!empty($tab['subs'])) {
            ...
            foreach ($tab['subs'] as $sub) {
              ...
              //试配加了badge之后的链接有效
              $sub_id    = $tab_id . '/' . sanitize_title(preg_replace("/<badge(.+?)<\/badge>/si",'',$sub['title']));
      ...
      foreach ($this->pre_sections as $section) {
        ...
        //试配加了badge之后的链接有效
        $section_parent = (!empty($section['ptitle'])) ? sanitize_title(preg_replace("/<badge(.+?)<\/badge>/si",'',$section['ptitle'])) . '/' : '';
        $section_slug   = (!empty($section['title'])) ? sanitize_title(preg_replace("/<badge(.+?)<\/badge>/si",'',$section_title)) : '';
      ...
      if (!empty($this->args['show_footer'])) {
        ...
        echo '<a href="javascript:;" class="csf-menu but hide-box c-yellow" title="显示选项菜单"><i class="fas fa-bars"></i></a>';
        echo '</div>';
