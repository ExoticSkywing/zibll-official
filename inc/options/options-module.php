 ...
 * @LastEditTime : 2026-01-02 12:58:46
...
<li>VIP用户专属QQ群</li>',
    ...
    public static function aut()
    {
        ...
        if (ZibAut::is_aut()) {
            ...
            <div>
                <p><span class="c-blue">授权域名</span><span class="c-blue badg">' . ZibAut::get_aut_url() . '</span></p>
                <input class="regular-text" type="text" ajax-name="aut_code" value="" placeholder="请输入授权码">
                <input type="hidden" ajax-name="action" value="admin_curl_aut">
            </div>
            <a id="authorization_submit" class="but c-blue ajax-submit curl-aut-submit" data-depend-id="zib_submit_aut">立即授权</a>
            <div class="ajax-notice"></div>
            </div>';
        }
        if (!ZibAut::is_local()) {
    ...
    public static function backup()
    {
        ...
        if ($options) {
            ...
            foreach ($options as $key => $val) {
                ...
                $del      = '<a href="javascript:;" ajax-url="' . add_query_arg('action', 'options_backup_delete', $ajax_url) . '" data-confirm="确认要删除此备份[' . $key . ']？删除后不可恢复！" class="but c-yellow ajax-get ml10">删除</a>';
                $restore  = '<a href="javascript:;" ajax-url="' . add_query_arg('action', 'options_backup_restore', $ajax_url) . '" data-confirm="确认将主题设置恢复到此备份吗？[' . $key . ']？" class="but c-blue ajax-get ml10">恢复</a>';
            ...
            if ($count > 3) {
                $delete_but = '<a href="javascript:;" ajax-url="' . add_query_arg(array('action' => 'options_backup_delete_surplus', 'key' => 'all'), $admin_ajax_url) . '" data-confirm="确认要删除多余的备份数据吗？删除后不可恢复！" class="but jb-red ajax-get">删除备份 保留最新三份</a>';
            }
        }
        $csf[] = array(
            'type'    => 'submessage',
            ...
            <a href="javascript:;" ajax-url="' . add_query_arg('action', 'options_backup', $admin_ajax_url) . '" class="but jb-blue ajax-get">备份当前配置</a>
            ' . $delete_but . '
