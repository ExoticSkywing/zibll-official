 ...
 * @LastEditTime : 2026-01-14 12:51:11
...
function zib_ajax_debounce($key, $args, $time = 3, $msg = '操作过于频繁，请%time%秒后再试')
{
    ...
    try {
        ...
        if (is_super_admin()) {
            zib_send_json_error(['msg' => '节流函数异常，可能会导致数据错误，请及时处理！错误信息：' . $e->getMessage() . '（此消息仅管理员可见）', 'error_msg' => $e->getMessage(), 'error_code' => $e->getCode(), 'error_file' => $e->getFile(), 'error_line' => $e->getLine()]);
        } else {
            return true;
        }
    }
