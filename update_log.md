...
-   新增知识付费(付费阅读、付费下载等)购买有效期功能，用户购买后在指定时间内可查看，到期后需重新购买
-   新增付费下载的每日下载免费资源超过限制后，可以单独购买的功能，并支持单独配置价格
-   新增自定义关注板块的关注名称，也就是修改关注这两个字，例如改为：加入、入驻、订阅等，更加适合将论坛作为圈子、社群等组织使用
-   新增板块发布权限和阅读权限都支持配置关注后允许，可实现例如：关注板块后才能查看、才能发帖，加入圈子后才能查看、发布等功能
-   新增付费关注板块功能，结合上面两个新功能，可实现付费加入圈子后才能查看、发布的效果，例如常见的付费圈子等
-   新增前台投稿设置付费下载时候，可直接粘贴网盘分享链接，多数分享链接可自动识别链接及提取码并自动填写
-   新增文章分类、专题、标签页面的搜索按钮，点击可在当前分类下快速搜索文章
-   新增论坛首页快速发帖功能

-   优化商品商城的封面图显示细节，点击有图片的选项时，商品封面图会自动切换到选项图
-   优化商城商品订单主图显示与商品选项图进行关联，更符合现代化商城体验
-   优化前台投稿、发帖时候，已经编写了内容后如果刷新页面会弹出提示确认，避免误触导致内容丢失
-   修复关闭 VIP 会员功能时，仍然会显示会员限购的 bug
-   修复搜索用户时，不能正确显示结果数量的 bug
-   修复删除商品评价后，商品评分不更新的 bug
...
-   新增论坛帖子的推荐指数排序方式，按照时间、阅读量、收藏量、评论量、评分与权重配置综合计算后进行排序，让用户感兴趣的内容始终排在前面
-   新增 Meilisearch 智能搜索功能，让搜索支持模糊搜索、智能分词，同时极大的提高搜索性能。
-   新增搜索结果筛选功能，支持自定义筛选项目，支持自定义排序，开启 Meilisearch 智能搜索后支持自动关联与结果相关的筛选，并显示关联数量
-   新增商城系统的推广返佣功能，支持统一设置，按分类设置、按商品单独设置，支持设置为按比例返佣，以及固定佣金金额
-   新增卡密自定义卡号和卡密显示名称的功能，例如将卡号自定义为账号，卡密自定义为授权码等
-   新增订单防刷单功能，按 10 分钟、按天限制下单频率，防止恶意刷单
-   新增自定义邮件标题前缀的功能
-   重构微信公众号模板消息功能

-   适配 WordPress V6.9.x 版本，解决 WP6.9 邮件发送失败的问题
-   优化 WP 后台速度，WP 后台会定期执行一些不必要的操作，且十分耗时，例如定期联网检查 PHP 版本、联网检查浏览器版本、检查 wp 贡献者等等，关闭后有效的提高后台速度
-   修复限购数量计算可能是错误的 bug
-   修复商品列表设置封面图长宽比例可能失效的 bug
-   修复搜索结果数量显示可能会错位的 bug
-   修复购物车页面，移动端底部可能会遮挡的 bug
-   修复后台举报处理页面可能会报错的 bug
-   修复视频剧集模块，在某些情况下可能会无法点击切换的 bug

### V8.4---2025/12/01

-   新增短信接口：阿里云号码认证-短信认证接口，此接口无需企业认证，无需申请资质、模板、签名，个人企业均可即可使用
-   新增邮箱、短信验证码验证频率限制功能，验证错误次数超过 5 次后限制 30 秒验证一次，错误超过 10 次后，限制 3 分钟验证一次，提高账户安全
-   新增用户在商城付款后自动显示付款成功弹窗，如果是自动发货的商品(如卡密等)将自动显示发货内容，让用户第一时间就能看到发货内容，提高用户体验
-   新增待审核帖子驳回并发消息通知作者的功能，方便通知作者整改
-   新增论坛帖子关闭评论功能，以及对应的权限管理配置
-   新增商品批量发货功能
-   新增商品发货后修改快递单号的功能
-   新增开启顶部多功能组件后，可以设置导航栏文字颜色的功能，当选择浅色图片作为背景时，设置深灰色文字颜色显示效果更佳

-   修复帖子列表小工具的阅读权限筛查功能失效的 bug
-   修复关闭用户等级功能后，论坛权限管理仍然会显示用户等级的 bug
-   修复代理登录的微信登录有时候可能失败的 bug
-   修复购物车同一件商品多个选项同时下单时，库存扣减可能会出错的 bug
-   修复商品列表默认设置为横向小卡片可能会失效的 bug
-   一些细节优化后和其他几个已知 bug 修复

### V8.3---2025/11/01

-   新增自动发货的商品可开启免登陆购买的功能
-   新增导入自定义卡密、生成自定义卡密以及销售自定义卡密的功能（支持配合免登陆购买）
-   新增商城销售自动发货的邀请码时，支持自动创建邀请码并发货的功能，实现无人值守，无需提前创建
-   新增商城商品手动输入购买数量的功能
-   新增顶部导航菜单直接设置图标、徽章的功能
-   新增顶部导航栏设置多栏目子菜单、图文卡片子菜单功能
-   新增右侧浮动按钮、移动端底部 Tab 按钮、顶部导航栏：页面向下滚动时隐藏的功能（向下滚动隐藏，向上滚动显示）
-   新增后台所有设置图标的项目，均支持添加自定义 SVG 图标代码功能
-   新增管理员可直接修改热门搜索关键词的功能（可直接在前台编辑）

-   优化大量 UI 显示细节
-   优化新版日夜间切换动画丝滑度，避免闪烁（请使用最新版 chrome 浏览器体验）
-   优化顶部和右侧浮动按钮的购物车按钮，未登录时候不显示
-   修复后台订单列表日期筛选可能会失效的 bug
-   修复一个微信登录可能会出错的 bug
-   修复后台新版订单列表可能会无法显示的 bug
-   修复微信 app 内付款时可能会报错的 bug
-   修复用户中心积分订单不显示金额的 bug
-   修复商城会员限购可能会失效的 bug
-   修复了其他的一些警告报错
...
-   新增实物商城系统
-   新增古腾堡：无缝图块，图片之间无缝连接，更加适合电商等场景
-   新增古腾堡：商品卡片块，方便在文章中插入商品
-   新增驳回帖子和删除帖子时可选择通知作者的功能，支持填写驳回或删除原因
-   新增分类、标签、帖子、板块等页面的前台管理员快速编辑功能
-   新增所有页面都可以单独启用顶部多功能组件的功能
-   新增文章列表缩略图填充方式可选 cover 和 contain 的选项及功能
-   全新的日夜间主题模式切换动画（仅支持部分新版浏览器）
-   全新的后台商城统计页面、订单页面
-   新增外链重定向防 CSRF 攻击功能，有效防止链接被恶意利用的情况

-   优化支付逻辑，优化用户中心订单列表
-   优化用户快捷评论时无需审核直接发布
-   优化 WP 后台菜单在移动端的操作体验，增加了左滑关闭功能
-   适配 WordPress 6.8.x 版本，修复古腾堡 tab 块无法使用的 bug
-   修复了一个 XSS 漏洞
-   修复了评论发高亮代码时有时候会不完整的 bug
-   全面适配 PHP7.0-8.5 版本，优化 PHP8 以上的性能(备注：不推荐使用 PHP8.2，其他版本性能都很快)
-   还有很多细节优化和 BUG 修复不一一展示了，详情可查看【需求提交】和【bug 提交】板块

### V8.1---2025/01/01

-   新增评论、私信的快捷回复功能，支持后台添加常用内容，支持用户自定义内容
-   新增收款接口自定义订单标题的功能
-   新增底部页面可添加自定义图标联系方式的功能
-   新增了多个社交平台的 icon 图标，可以在主题设置中选择使用
-   新增限制用户设置付费金额区间和积分金额区间的功能
-   新增多栏目论坛帖子小工具模块
-   链接列表小工具、链接列表页面模板新增是否添加 nofollow 标记的选项及功能
-   新增古腾堡编辑器块：剧集嵌入块，可以像视频剧集一样将多个嵌入地址组合成剧集
-   为文章相关的小工具模块，新增排序可选择升序或倒序的功能

-   更新适配 WordPress 6.7.1 版本
-   更新付费资源对 123 网盘新链接的的自动识别
-   更新腾讯人机验证功能为腾讯天御验证码 2.0 版
-   修复发帖选择标签时可能会无法添加的 bug
-   修复微信公众号登录在移动端截图扫码登录时可能会无法跳转的 bug
-   修复视频模块退出全屏后可能会出现页面位置不正确的 bug
-   修复文章列表小工具某些情况下可能不能对齐的 bug
-   修复用户个人信息小工具未登录情况下无法关闭封面的 bug
