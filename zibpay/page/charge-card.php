 ...
 * @LastEditTime : 2025-12-23 21:04:08
 */
    ...
    switch ($action) {
        case 'add':
            ...
            if ($add_type === 'import') {
                    ...
                    switch ($pass_type) {
                        ...
                        case 'custom':
                            ...
                            if ($only_pass_s) {
                                ...
                                $custom_name = !empty($v[2]) ? wp_parse_args($v[2]) : [];
                            } else {
                                $_other      = !empty($v[2]) ? trim($v[2]) : '';
                                $custom_name = !empty($v[3]) ? wp_parse_args($v[3]) : [];
                            }

                            if (!empty($custom_name['card_name'])) {
                                $pass_meta['custom_name']['card_name'] = trim($custom_name['card_name']);
                            }
                            if (!empty($custom_name['password_name'])) {
                                $pass_meta['custom_name']['password_name'] = trim($custom_name['password_name']);
                            }
                ...
                $custom_name = !empty($_REQUEST['custom_name']) ? $_REQUEST['custom_name'] : [];
                if (!empty($custom_name['card_name'])) {
                    $pass_meta['custom_name']['card_name'] = trim($custom_name['card_name']);
                }
                if (!empty($custom_name['password_name'])) {
...
switch ($tab) {
    ...
    case 'add':
        ...
        $csf_fields[] = array(
            'dependency' => array('add_type|type', '==|==|==', 'auto|custom'),
            'title'      => '自定义显示名称',
            'id'         => 'custom_name',
            'type'       => 'fieldset',
            'fields'     => array(
        ...
        $csf_fields[] = array(
            ...
            <li>一行一个卡密，单行格式为：<code>卡号 密码 备注 自定义参数</code></li>
            <li>默认使用空格分割，您可以在下方自定义分割符号，与您的数据对应即可</li>
            <li>自定义参数格式为：<code>card_name=授权号&password_name=授权密码</code>,card_name为卡号的显示名称，password_name为卡密的显示名称，可以留空</li>
            <li>单次导入数量太多可能会对服务器性能造成影响</li>
            <p><b>数据示例</b></p>
            <div>4849977843617 3w20V4Qe7dRhAIoPYum6KSA 平台卡密01 card_name=授权号&password_name=授权码</div>
            <div>6484027668705 SkdJZ5vWcpqPxrbs3pqPcpq 平台卡密01 card_name=授权号&password_name=授权码</div>
            <div>111111@qq.com 11assssddsds 某某账号 card_name=账号&password_name=密码</div>',
            'style'      => 'warning',
            'type'       => 'submessage',
        );
        $csf_fields[] = array(
            ...
            <li>一行一个卡密，单行格式为：<code>密码 备注 自定义参数</code></li>
            <li>默认使用空格分割，您可以在下方自定义分割符号，与您的数据对应即可</li>
            <li>单次导入数量太多可能会对服务器性能造成影响</li>
            <li>自定义参数格式为：<code>password_name=授权密码</code>，password_name为卡密的显示名称，可以留空</li>
            <p><b>数据示例</b></p>
            <div>3w20V4Qe7dRhAIoPYum6KSA 平台卡密01 password_name=授权码</div>
            <div>SkdJZ5vWcpqPxrbs3pqPcpq 平台卡密01 password_name=授权码</div>',
            'style'      => 'warning',
            'type'       => 'submessage',
        );
        $csf_fields[] = array(
        ...
        if ($db_data) {
            ...
            foreach ($db_data as $msg) {
                ...
                if (!empty($meta['custom_name'])) {
                    ...
                    if (!empty($custom_name['card_name'])) {
                        $card_name = $custom_name['card_name'];
                    }
                    if (!empty($custom_name['password_name'])) {
                        $password_name = $custom_name['password_name'];
                    }
                }
                ...
                $tbody .= "<td><span data-clipboard-text='$msg->card' data-clipboard-tag=" . ($card_name ?: '卡号') . ">$card_name $msg->card</span></td>";
                $tbody .= "<td><span data-clipboard-text='$msg->password' data-clipboard-tag=" . ($password_name ?: '密码') . ">$password_name $msg->password</span></td>";
